...
| `describedBy`        | `described-by`        | Id of the element that describes the button, set as aria-describedby.                                                                                                                                                                                                                                          | `string`                                                                                                                                         | `undefined` |
...
| `expanded`           | `expanded`            | Whether what the button controls is open, announced as aria-expanded. Set by whatever it opens.                                                                                                                                                                                                                | `boolean`                                                                                                                                        | `undefined` |
...
| `join`               | `join`                | Flattens the corners on the side that meets a neighbour, so a group can present several controls as one segmented unit. A wrapper sets this rather than reaching into the shadow root, which nothing outside it can style.                                                                                     | `"both" \| "end" \| "start"`                                                                                                                     | `undefined` |
...
| `popup`              | `popup`               | What the button opens, announced as aria-haspopup. Set by `ss-popover` and `ss-dropdown` on their trigger. A button that opens something is not an action that can be sent twice, so it skips the post-click disable: focus handed back to it on close would otherwise land on a disabled control and be lost. | `"dialog" \| "listbox" \| "menu"`                                                                                                                | `undefined` |
