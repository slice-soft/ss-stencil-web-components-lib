...
describe('ss-toaster', () => {
  it('is a region named for what it holds', async () => {
    ...
    const region = getRoot(page).querySelector('section')!;
  ...
  it('takes a stated name', async () => {
    ...
    expect(getRoot(page).querySelector('section')?.getAttribute('aria-label')).toBe('Avisos');
  });

  it('defaults to the bottom-end corner, and takes another', async () => {
    const page = await newSpecPage({ components: [SsToaster], html: `<ss-toaster></ss-toaster>` });
    expect(getRoot(page).querySelector('section')?.className).toContain('ss-toaster--bottom-end');

    const top = await newSpecPage({ components: [SsToaster], html: `<ss-toaster placement="top-center"></ss-toaster>` });
    expect(getRoot(top).querySelector('section')?.className).toContain('ss-toaster--top-center');
  });
});
